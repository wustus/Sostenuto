...
def  main():
  ...
  installer = "\installer\Sostenuto Installer.exe"
   
  if demo:
    installer = "\installer\Sostenuto Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Sostenuto manual.pdf" 
  ]
  ...
  ZIPNAME = "Sostenuto-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Sostenuto-v" + FULLVERSIONSTR + "-win-demo.zip"
